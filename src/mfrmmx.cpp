...
#include "update_theta_integrated_ind.h" 
...
////* mfrmmx_dep *////
// [[Rcpp::export]]
Rcpp::List mfrmmx_dep(const Rcpp::List Y,        // observations
                      ...
                      const double at=1,                    // tau2 prior
                      const double bt=0,                    // tau2 prior
                      const double at0=1,                   // bt prior
                      ...
                      const int burn_eps=1e4,               // MCMC settings
  ...
  Rprintf("burn_eps = %i\n", burn_eps);
  Rprintf("burn = %i\n", burn);
  const int burn_tot = burn_eps + burn;
  Rprintf("burn_tot = %i\n", burn_tot);
  Rprintf("thin = %i\n", thin);
  Rprintf("ns = %i\n", ns);
  Rprintf("\n");
  ...
  while (s < ns) {
    ...
    if (SM == 1) {
      ...
      for (int l = 0; l < 2; l++) {
        for (int d = 0; d < D; d++) {
          tau2_s(l,d) = 1/Rf_rgamma(at,1/btD(d));
      ...
      for (int d = 0; d < D; d++) {
        tau2_m(d) = 1/Rf_rgamma(at,1/btD(d));
      ...
      if (zi != zj) {
        ...
        /* create new model parameters for the new proposed empty cluster */
        /* from their prior distributions                                 */
        arma::vec lam2_m_empty(D);
        arma::vec tau2_m_empty(D);
        arma::mat theta_m_empty(P,D);
        
        for (int d = 0; d < D; d++) {
          tau2_m_empty(d) = 1/Rf_rgamma(at,1/btD(d));
          theta_m_empty.col(d) = rmvnorm(1, mu.col(d) , tau2_m(d)*Ki).t();
          double lam = 0.0;
          while (lam < lam_min) lam = Rf_runif(0.0,A);
          lam2_m_empty(d) = lam*lam;
        }
        
        
        
        // /* prior log ratio (Eq. (8)) - Repulsive factor */ // (OLD ERROR: without distances of the new empty)
        // if (fmmx==1) {
        //   if (repulsive != 0) {
        //     
        //     double log_RF = 0;
        ...
        //     for (int d = 0; d < D; d++) {
        //       
        //       // theta matrix without column zj
        //       arma::mat theta_ = theta.slice(d);
        //       theta_.shed_col(zj);
        //       
        //       // difference between curve zj and other curves
        ...
        if (fmmx == 1) {
          if (repulsive != 0) {
            ...
            for (int d = 0; d < D; d++) {
              ...
              // difference between curves zi and zj
              arma::mat df_m_zij(n_theta,1);
              df_m_zij.col(0) = Ht0*(theta_m_empty.col(d) - theta_m.col(d));
              arma::mat df_zij(n_theta,1);
              df_zij.col(0) = Ht0*(theta.slice(d).col(zi) - theta.slice(d).col(zj));
              
              // difference between curve zj and other curves
              arma::mat df_m(n_theta,J-2);
              for (int h = 0; h < J-2; h++) df_m.col(h) = Ht0*(theta_m.col(d) - theta_.col(h));
              arma::mat df_zj(n_theta,J-2);
              for (int h = 0; h < J-2; h++) df_zj.col(h) = Ht0*(theta.slice(d).col(zj) - theta_.col(h));
              
              // difference between curve zi and other curves
              arma::mat df_m_empty(n_theta,J-2);
              for (int h = 0; h < J-2; h++) df_m_empty.col(h) = Ht0*(theta_m_empty.col(d) - theta_.col(h));
              arma::mat df_zi(n_theta,J-2);
              for (int h = 0; h < J-2; h++) df_zi.col(h) = Ht0*(theta.slice(d).col(zi) - theta_.col(h));
              
              // repulsive factor (log)
              log_RF += logRF(repulsive, phi, nu, r, n_theta, df_m_zij);
              log_RF -= logRF(repulsive, phi, nu, r, n_theta, df_zij);
              
              log_RF += logRF(repulsive, phi, nu, r, n_theta, df_m);
              log_RF -= logRF(repulsive, phi, nu, r, n_theta, df_zj);
              
              log_RF += logRF(repulsive, phi, nu, r, n_theta, df_m_empty);
              log_RF -= logRF(repulsive, phi, nu, r, n_theta, df_zi);
        ...
        if (log(u) < log_alpha) {
          ...
          for (int d = 0; d < D; d++) {
            ...
            if (fmmx==1) {
              ...
              // // empty cluster zi
              // tau2(zi,d) = 1/Rf_rgamma(at,1/btD(d));
              ...
              // empty cluster zi
              tau2(zi,d) = tau2_m_empty(d);
              theta.slice(d).col(zi) = theta_m_empty.col(d);
              lam2(zi,d) = lam2_m_empty(d);
    ...
    if (b == burn_eps) Rprintf("burn epsilon finished :)\n\n");
    if (b == burn_tot) Rprintf("burn total finished :)\n\n");
  ...
  out["v"] = v;
  out["ns"] = ns;
  out["burn_eps"] = burn_eps;
  out["burn"] = burn;
  out["burn_tot"] = burn_tot;
  out["thin"] = thin;
  if (fmmx == 1) out["J"] = J;
  out["at"] = at;
  if (bt > 0) out["bt"] = bt;
...
////* mfrmmx_ind *////
// [[Rcpp::export]]
Rcpp::List mfrmmx_ind(const Rcpp::List Y,               // observations
                      const int D,                          // number of curves of each individual 
                      const arma::mat X,                    // LSBP - covariates matrix
                      const arma::vec mu_alpha,             // LSBP - mean of alpha prior
                      const arma::mat S_alpha,              // LSBP - variance of alpha prior
  ...
  while (s < ns) {
    ...
    if (SM == 1) {
      ...
      if (zi != zj) {
        ...
        //     for (int d = 0; d < D; d++) {
        //       
        //       // theta matrix without columns zj
        //       arma::mat theta_ = theta.slice(d);
        //       theta_.shed_col(zj);
        //       
        //       // difference between curve zj and other curves
    ...
    for (int d = 0; d < D; d++) {
      ...
      eps = epsD(d);
      eps0 = eps0D(d);
      update_theta_ind(theta, accept_mat, accept_logratio_mat,
                       nj, lam2, tau2, beta0, sig2, mu, z, K, Ki,
                       m, d, J, P, Ip, Y, H, HH, n_theta, Ht0, theta_sampler,
                       repulsive, phi, nu, r, nr, eps, eps0);
